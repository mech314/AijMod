...
void sendCanMessage(unsigned char ccm)
{
	...
	{		
		...
		// Execute callback after sending the message if one is defined
        if(ccmGroup[ccm].callback > 0)
        {
            if(ccmGroup[ccm].callback % 4 == 0) // Only call it if it's U32 aligned
            {
                void (*callbackPtr)(void) = (void*)ccmGroup[ccm].callback;
                (*callbackPtr)();
            }
        }
    }
...
void counterCallBack(void) {
    // Increment the counter from 0-255 (will automatically wrap due to unsigned char)
    pRamVariables.m421_b3++;
}
