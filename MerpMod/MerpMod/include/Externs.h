...
void counterCallBack(void) ROMCODE;
...
//unsigned char counter(unsigned char input) ROMCODE; // Counter for ATemu
...
float SwitchSelect(TableSubSet tss, float xLookup, float yLookup) ROMCODE; // Pass table address and axis values to return the float from the table? 
